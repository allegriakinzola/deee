...
import { InvitePartnerMemberForm } from "@/components/partner/invite-partner-member-form"
import {
  ACCESS_LABELS,
  filterPartnerUsers,
  initials,
  summarizePartnerUsers,
  type PartnerUsersViewFilter,
} from "@/components/partner/partner-user-copy"
...
export function PartnerUsersWorkspace({
  actorId,
  partnerId,
  users,
}: {
  ...
  const stats = useMemo(() => summarizePartnerUsers(users), [users])
  const [filter, setFilter] = useState<PartnerUsersViewFilter>("all")
  const [query, setQuery] = useState("")
  const [createOpen, setCreateOpen] = useState(false)
  const [created, setCreated] = useState<PartnerInvitationResult | null>(null)
  const [copied, setCopied] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<DirectoryPartnerAdmin | null>(
    null
  )
  ...
  const visible = useMemo(
    () => filterPartnerUsers(users, filter, query),
    [users, filter, query]
  )

  function selectFilter(next: PartnerUsersViewFilter) {
  ...
  async function setStatus(
    ...
    const result = await patchJson(
      `/api/v1/partners/${partnerId}/members/${user.id}/status`,
      { status }
    )
    setPendingId(null)
  ...
  async function resendInvite(user: DirectoryPartnerAdmin) {
    if (!user.email) {
  ...
  async function confirmDelete() {
    ...
    const result = await deleteJson(
      `/api/v1/partners/${partnerId}/members/${deleteTarget.id}`
    )
    setPendingId(null)
    setPendingAction(null)
    if (!result.ok) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-muted-foreground">
            Invitez les administrateurs de votre entreprise. Ils activeront
            leur compte par e-mail.
          </p>
        </div>
        <Button
          type="button"
      ...
      >
        ...
        >
          <SheetHeader className="border-b border-border/80">
            <SheetTitle>
              {created ? "Invitation envoyée" : "Inviter un administrateur"}
            </SheetTitle>
            <SheetDescription>
              {created
                ? created.emailed
                ...
                : "Un administrateur de votre entreprise. Le mot de passe se définit à l’acceptation."}
            </SheetDescription>
          </SheetHeader>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteTarget
                ? `${deleteTarget.displayName} sera retiré de votre entreprise. Les invitations qu’il a envoyées resteront, rattachées à votre compte.`
                : null}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
